...
## 0. Принцип хранения данных и миграции на Supabase

GotMeds использует реляционную модель данных на PostgreSQL. Целевое состояние проекта — размещение и управление основной БД через Supabase PostgreSQL.

На текущем переходном этапе схема может поддерживаться через `prisma/schema.prisma` и Prisma migrations. После отдельного утвержденного плана источником изменений схемы должны стать Supabase SQL migrations. До завершения этого перехода Prisma остается рабочим слоем и не удаляется.

Миграция на Supabase не должна менять доменную модель без отдельной спецификации. Таблицы, связи, enum-значения, индексы и ограничения должны сохранять совместимость с существующими пользовательскими сценариями.
...
## 3. Требования для целевого Supabase-состояния

### 3.1. SQL migrations

После этапа переноса миграций структура БД должна воспроизводиться через Supabase SQL migrations:
...
Целевое состояние админской авторизации — Supabase Auth. Доменные роли администратора (`superadmin`, `content_manager`) должны быть связаны с Supabase Auth user через отдельную таблицу профиля или через проверяемые claims.

До утвержденного этапа Supabase Auth таблица `Admins` и текущая серверная авторизация остаются допустимым переходным состоянием.
