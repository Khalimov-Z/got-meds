...
### 1.1. Целевой Supabase data layer

В рамках стратегии [supabase-platform-migration.md](./supabase-platform-migration.md) контракты этого документа должны постепенно перейти с Prisma-реализаций на Supabase data layer:

1. **Supabase SDK:** используется для простых чтений и записей, которые безопасно покрыты RLS-политиками.
2. **SQL functions / RPC:** используются для сложной бизнес-логики рядом с данными: fuzzy search, выборка аптек, подбор аналогов, агрегации аналитики и атомарные админские операции.
3. **Server Actions как граница безопасности:** сохраняются для сценариев, где нужна серверная валидация, работа с `service_role`, обработка файлов или композиция нескольких операций.

До завершения соответствующих этапов миграции текущие Prisma-реализации этих контрактов остаются допустимым рабочим состоянием.
...
* **Целевое Supabase-состояние:** перенос логики fuzzy search в SQL function/RPC, чтобы поиск выполнялся внутри Supabase PostgreSQL и сохранял единый контракт ответа.
...
* **Целевое Supabase-состояние:** перенос выборки в SQL function/RPC с проверкой статуса аптеки, времени работы, tier-фильтра и расчета расстояния на стороне БД.
* **Выходные данные (JSON):**
  ```typescript
  Array<{
    pharmacy_id: string;
...
* **Описание:** Запускается асинхронно в фоне, когда пользовательский поиск ничего не нашел. Записывает данные в таблицу `Search_Logs` для B2B дашборда админов.
* **Антишум:** функция не должна вызываться на каждый промежуточный live-search запрос. Логируется только стабилизированный пустой запрос: строка длиной не менее 3 символов, которая не менялась минимум 1.5 секунды после показа пустой выдачи. Restricted-запросы не логируются как дефицит.
* **Целевое Supabase-состояние:** запись может выполняться через Server Action или RPC с RLS/антиспам-ограничениями. Restricted-запросы по-прежнему не должны попадать в аналитику дефицита.
...
* **Целевое Supabase-состояние:** запись жалобы должна быть защищена RLS-политикой и серверной антиспам-проверкой. Пользователь не должен иметь возможность менять статус жалобы или читать админскую очередь модерации.
...
* **Маппинг CSV:** `uploadPharmacyPrice(pharmacyId: string, fileData: Blob)`
  ...
  * В целевом Supabase-состоянии выполняется через Server Action с серверным Supabase-клиентом и RPC/транзакционной SQL-функцией.
* **Привязка Алиаса:** `createAlias(unmappedStringId: string, productId: string)`
  ...
  * В целевом Supabase-состоянии требует админскую роль и RLS/RPC-проверку.
