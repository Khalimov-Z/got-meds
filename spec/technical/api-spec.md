...
### 1.1. Supabase data layer

Контракты этого документа реализуются через Supabase data layer:

1. **Supabase SDK:** используется для простых чтений и записей, которые покрыты RLS-политиками или выполняются на server-only границе.
...
* **Supabase-состояние:** логика fuzzy search выполняется в SQL function/RPC внутри Supabase PostgreSQL и сохраняет единый контракт ответа.
...
* **Supabase-состояние:** выборка выполняется в SQL function/RPC с проверкой статуса аптеки, времени работы, tier-фильтра и расчета расстояния на стороне БД.
* **Выходные данные (JSON):**
  ```typescript
  Array<{
    pharmacy_id: string;
...
* **Supabase-состояние:** запись выполняется через Server Action или RPC с RLS/антиспам-ограничениями. Restricted-запросы по-прежнему не должны попадать в аналитику дефицита.
