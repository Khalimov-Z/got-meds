...
- Области: инфраструктура и качество, frontend и дизайн, backend и API
- Сделано: проект успешно развернут на Vercel (https://got-meds.vercel.app/) с подключением к существующей облачной базе данных Supabase; настроены все необходимые переменные окружения; исправлен отступ иконки бренда (BrandMark) для точного оптического выравнивания по левому краю; устранена ошибка линтера React-hooks (set-state-in-effect) в компоненте SearchExperience.
- Проверка: автоматические проверки сборки, линтинга и тестов пройдены успешно; проверены ключевые пользовательские сценарии (поиск, карта, вход в админку с редиректом) на живом сайте в Vercel.
- Pull request: будет создан после push
...
- Области: данные, поиск и бизнес-логика, backend и API, frontend и дизайн
- Сделано: исправлен баг, при котором кнопки «Медтехника», «Витамины», «Мать и дитя» вставляли текст в строку поиска вместо фильтрации по категории; добавлен параметр `p_category` в SQL-функцию `gotmeds_search_products`, пробошен через API и UI; кнопки теперь возвращают все товары указанной категории.
- Проверка: пользователь вручную проверил все три кнопки категорий и текстовый поиск; выполнен `npm run build`.
- Pull request: будет создан после push
