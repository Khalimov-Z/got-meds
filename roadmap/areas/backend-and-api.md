...
- Сделано: обновлены типы `ProductDetailsRpcRow` для поддержки `is_social_risk`, модифицированы Server Actions `getProductDetails`, `getAnalogs` и `getPharmaciesByProduct` (возвращают статус `restricted` и ошибку при попытке получить данные заблокированного препарата); `sitemap.xml` корректно исключает заблокированные продукты из карты сайта.
- Влияние: бэкенд-слой гарантирует, что чувствительные и заблокированные препараты исключены из аналогов, sitemap и карты аптек, а прямой запрос деталей возвращает статус ограничения.
- Проверка: протестированы Server Actions с помощью unit-тестов (`npm run test:unit`), а также ручным переходом по UUID заблокированного товара.
...
- Сделано: Внедрены Server Actions `submitPartnerRequestAction` для публичной отправки заявок, `getPartnerRequestsAction` и `updatePartnerRequestStatusAction` для административного интерфейса; обновлен подсчет новых заявок на дашборде админки в `getAdminHomeStats`; настроена валидация входных данных, безопасная обработка SQL-ошибок (RPC недоступен) с выдачей понятных русскоязычных сообщений, а обновление статуса переведено на авторизованный клиент `createSupabaseAuthServerClient` для передачи JWT администратора.
- Влияние: Создан надежный API-слой для отправки и модерации B2B-заявок с защитой RLS и разграничением прав.
- Проверка: Протестирована отправка заявок и обновление статусов через Jest-тесты в `tests/unit/actions/partner.test.ts`.
...
- Сделано: Упрощена и очищена документация миграций в `supabase/README.md` (удалены специфичные проверочные SQL-скрипты, оставлена только базовая документация); в `search-experience.tsx` интегрирован ResizeObserver на клиенте Next.js.
- Влияние: README стал устойчивым к изменениям схемы базы данных и готов к долгосрочному использованию без частых ручных правок.
- Проверка: Успешно пройден билд проекта `npm run build` с новыми типами и стилями.

## 2026-06-18 - Исправление кнопок быстрого поиска по категориям

- План: [plans/completed/fix-category-quick-search.md](../../plans/completed/fix-category-quick-search.md)
- Сделано: `searchProducts` принимает необязательный параметр `category` и передаёт его как `p_category` в RPC; `GET /api/search` принимает query-параметр `category` и валидирует наличие хотя бы одного из `q` или `category`.
- Влияние: API поиска поддерживает фильтрацию по категории без изменения существующих текстовых запросов.
- Проверка: пользователь проверил кнопки категорий и текстовый поиск; выполнен `npm run build`.
