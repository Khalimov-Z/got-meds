...
- Сделано: активный план переведен в `plans/completed/`, `Montserrat` для логотипа подключен локальным `.woff2` через `next/font/local`, чтобы новый брендовый шрифт не добавлял новую зависимость от Google Fonts на этапе сборки.
- Влияние: брендовый шрифт воспроизводимо поставляется вместе с приложением; существующая зависимость сборки от Google Fonts остается только у прежнего `Hanken Grotesk`.
- Проверка: выполнены `npm run lint`, `npm run build` с сетевым доступом для существующего `Hanken Grotesk` и `git diff --check`; сборка без сети ожидаемо падает на прежнем `Hanken Grotesk`, а не на новом `Montserrat`.
...
- Сделано: внедрены unit-тесты для новых Server Actions (управление алиасами, удаление алиасов) и проверена отказоустойчивость импорта прайс-листов.
- Влияние: качество кодовой базы защищено автоматическими регрессионными Jest-тестами, повышена безопасность импорта.
- Проверка: успешно выполнены `npm run test:unit`, `npm run lint` и `npm run build`.
...
- Сделано: добавлены 4 unit-теста в `products.test.ts` и `product-page.test.ts` для проверки блокировок товаров в Server Actions (детали, аналоги, аптеки) и для проверки robots-метатегов на странице товара.
- Влияние: исключена регрессия правил черного списка при дальнейшей разработке.
- Проверка: успешно выполнены Jest-тесты (`npm run test:unit` возвращает 27/27 pass), линтинг (`npm run lint`) и production-сборка (`npm run build`).
...
- Сделано: Успешно проведена production-сборка (`npm run build`) с новыми CSS-стилями и ResizeObserver, гарантируя отсутствие регрессии типов или поломки SSR-страниц.
- Влияние: Качество интерфейса и стабильность сборки подтверждены автоматизированными компиляционными проверками.
- Проверка: Выполнен `npm run build` с успешным результатом.


## 2026-06-18 - Деплой на Vercel и подключение к Supabase

- План: [plans/completed/vercel-supabase-deployment.md](../../plans/completed/vercel-supabase-deployment.md)
- Сделано: проект успешно развернут на Vercel с подключением к существующей облачной базе данных Supabase, а в `search-experience.tsx` устранена ошибка линтера React-hooks (set-state-in-effect) с помощью requestAnimationFrame.
- Влияние: подтверждена работоспособность сборки в production-окружении, проект доступен пользователям по сети.
- Проверка: автоматические проверки сборки (`npm run build`), линтинга (`npm run lint`) и unit-тестов пройдены успешно; проведена ручная проверка в браузере.
