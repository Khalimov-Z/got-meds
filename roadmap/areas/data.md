...
- Сделано: дополнена спецификация пайплайнов данных [data-pipelines.md](../../spec/features/data-pipelines/data-pipelines.md), регламентирующая поведение при падении объёма остатков. В Server Action `uploadPharmacyPrice` внедрена предварительная выборка количества текущих записей `inventory` аптеки для сравнения со свежими импортируемыми данными.
- Влияние: данные остатков в базе защищены от случайной очистки при некорректном разборе или загрузке пустых файлов.
- Проверка: выполнена ручная проверка блокировки импорта пустого файла и успешная принудительная очистка остатков при установленной галочке «Принудительная загрузка».
...
- Сделано: создана Supabase SQL-миграция `supabase/migrations/20260604120000_blacklist_details.sql`, расширяющая RPC-функцию `gotmeds_get_product_details` для возврата признака `is_social_risk`.
- Влияние: клиентский слой Next.js получает поле безопасности напрямую из базы при запросе деталей продукта, сохраняя изоляцию.
- Проверка: миграция успешно применена пользователем в Supabase SQL Editor, новые колонки проверены в работе с тестовым UUID.
...
- Сделано: создана SQL-миграция `supabase/migrations/20260618120000_search_category_filter.sql`, добавляющая параметр `p_category` в `gotmeds_search_products` для фильтрации товаров по категории (`EQUIPMENT`, `VITAMINS`, `MOTHER_AND_BABY`).
- Влияние: функция поиска поддерживает три режима: только текст, только категория и текст + категория; обратная совместимость сохранена.
- Проверка: миграция применена пользователем в Supabase SQL Editor; три кнопки категорий проверены вручную.
