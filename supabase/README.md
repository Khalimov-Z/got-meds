...
Prisma остается рабочим rollback-слоем и seed-слоем до отдельного этапа Prisma decommission. Этап read layer переводит публичные и аналитические чтения на server-only Supabase SDK/RPC. Этап Auth/RLS переводит вход в админку на Supabase Auth и включает RLS. Этап admin mutations переводит текущие админские операции на Supabase SDK/RPC, но не удаляет Prisma.
...
Проверка приложения не дублируется полностью в этом runbook. Для текущего этапа используйте пользовательский чеклист в `plans/active/supabase-admin-mutations.md`; после завершения этапа — соответствующий файл в `plans/completed/`.
